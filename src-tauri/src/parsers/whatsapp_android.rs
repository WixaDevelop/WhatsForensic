//! Parser de WhatsApp Android sobre `msgstore.db`.
//!
//! Esquema post-2021. Timestamps en Unix milisegundos. Mapeos en
//! `resources/schemas/whatsapp_android/unknown.toml`, todos seed (`verified=false`).
...
const SEED_TOML: &str = include_str!("../../resources/schemas/whatsapp_android/unknown.toml");
...
pub struct WhatsAppAndroid {
    mapping: Mapping,
}

impl WhatsAppAndroid {
    pub fn new() -> Result<Self, AppError> {
        let mapping: Mapping = toml::from_str(SEED_TOML).map_err(|e| {
            AppError::new(
                AppErrorKind::Internal,
                "MAPPING_PARSE_FAILED",
                format!("No se pudo parsear el mapping seed Android: {e}"),
            )
        })?;
        Ok(Self { mapping })
    }
...
impl Parser for WhatsAppAndroid {
    ...
    fn detect(&self, filename: &str, schema: &SchemaSnapshot) -> Confidence {
        let filename_hint = filename.to_lowercase().contains("msgstore");
        let has_message = schema.tables.contains_key("message");
        let has_chat = schema.tables.contains_key("chat");
        match (filename_hint, has_message, has_chat) {
    ...
    fn parse(&self, conn: &Connection) -> Result<ParsedEvidence, AppError> {
        ...
        let mut conv_index: HashMap<i64, String> = HashMap::new();
        {
            let mut stmt = conn
                .prepare(
                    "SELECT _id, COALESCE(subject, ''), COALESCE(jid, '') FROM chat ORDER BY _id",
                )
                .map_err(map_err)?;
            let rows = stmt
                .query_map([], |r| {
                ...
                .map_err(map_err)?;
            for row in rows {
                let (pk, subject, jid) = row.map_err(map_err)?;
        ...
        let mut stmt = conn
            .prepare(
                "SELECT _id, chat_row_id, key_id, key_remote_jid, key_from_me, \
                        status, data, timestamp, message_type \
        ...
        while let Some(row) = rows.next().map_err(map_err)? {
            ...
            let chat_row_id: Option<i64> = row.get(1).map_err(map_err)?;
            let _key_id: Option<String> = row.get(2).map_err(map_err)?;
            let remote_jid: Option<String> = row.get(3).map_err(map_err)?;
            let is_from_me: Option<i64> = row.get(4).map_err(map_err)?;
            let _status: Option<i64> = row.get(5).map_err(map_err)?;
            let body: Option<String> = row.get(6).map_err(map_err)?;
            let ts_raw: Option<i64> = row.get(7).map_err(map_err)?;
            let msg_type_raw: Option<i64> = row.get(8).map_err(map_err)?;
            ...
            let sender = match direction {
                MessageDirection::Outgoing => None, // El sender outgoing es el owner del dispositivo.
                _ => remote_jid.clone(),
            };
            ...
            let ts_raw_f = ts_raw.map(|v| v as f64);
            let ts_utc = ts_raw_f.and_then(|v| convert(v, TimestampFormat::UnixMs));
            ...
            let conv_id = chat_row_id
                .and_then(|id| conv_index.get(&id).cloned())
                .unwrap_or_else(|| "conv:orphan".to_string());

            if let Some(id) = chat_row_id {
                *counts_per_conv.entry(id).or_insert(0) += 1;
            }
            ...
            out.messages.push(Message {
                ...
                timestamp_raw: ts_raw_f,
                timestamp_raw_format: Some("unix_ms".to_string()),
                sender,
                direction,
                body,
                media_ref: None,
...
fn map_err(e: rusqlite::Error) -> AppError {
    AppError::new(
        AppErrorKind::Io,
        "PARSER_QUERY_FAILED",
        format!("Consulta del parser WhatsApp Android falló: {e}"),
    )
}
